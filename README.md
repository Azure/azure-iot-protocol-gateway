...
- **SubscriptionCreationTimeCheckThreshold**. If specified, the time period threshold for time variances between the Protocol Gateway server and the IoT Hub server when checking if a subscription existed when C2D messages were enqueued on the IoT Hub. Default value: 00:00:00
