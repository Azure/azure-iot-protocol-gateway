namespace ProtocolGateway.Host.Fabric.BackEnd
{
    ...
    internal sealed class BackEnd : StatefulService, IBackEndService
    {
        ...
        {
            var sessionStateDictionary = await this.StateManager.GetOrAddAsync<IReliableDictionary<string, ReliableSessionState>>(this.backEndConfigurationProvider.Config.SessionDictionaryName);
            using (var tx = this.StateManager.CreateTransaction())
            {
                await sessionStateDictionary.AddOrUpdateAsync(tx, identityId, sessionState, (s, state) => sessionState);
        ...
        public async Task DeleteSessionStateAsync(string identityId)
        {
            ...
            using (var tx = this.StateManager.CreateTransaction())
            {
                await sessionStateDictionary.TryRemoveAsync(tx, identityId);
                await tx.CommitAsync();
            }
        }
        ...
        public async Task<ReliableSessionState> GetSessionStateAsync(string identityId)
        {
            ...
            using (var tx = this.StateManager.CreateTransaction())
            {
                var result = await sessionStateDictionary.TryGetValueAsync(tx, identityId);
        ...
        public async Task<ReliableMessageDeliveryState> GetMessageAsync(string identityId, int packetId)
        {
            ...
            var deliveryStateDictionary = await this.StateManager.GetOrAddAsync<IReliableDictionary<string, ReliableMessageDeliveryState>>(this.backEndConfigurationProvider.Config.Qos2DictionaryName);
            using (var tx = this.StateManager.CreateTransaction())
            {
                var result = await deliveryStateDictionary.TryGetValueAsync(tx, key);
        ...
        public async Task DeleteMessageAsync(string identityId, int packetId)
        {
            ...
            using (var tx = this.StateManager.CreateTransaction())
            {
                await deliveryStateDictionary.TryRemoveAsync(tx, key);
                await tx.CommitAsync();
            }
        }
        ...
        public async Task SetMessageAsync(string identityId, int packetId, ReliableMessageDeliveryState message)
        {
            ...
            using (var tx = this.StateManager.CreateTransaction())
            {
                await deliveryStateDictionary.AddOrUpdateAsync(tx, key, message, (s, state) => message);
                await tx.CommitAsync();
            }
        }
