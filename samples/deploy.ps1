...
function Execute-Command([scriptblock]$Command, [Alias('EEL')][Array]$ExpectedExceptionList, $maxCommandRetries=5, $writeStatus=$true)
{
    ...
    do {
        ...
        {
            Write-Verbose "$(Get-Date -f $timeStampFormat) - Exception occurred while executing $($Command.ToString()). Exception is: $($ex.Exception.GetType().Name) - $($ex.Exception.Message)"
...
if (!$storageAccount)
{
    $storageAccount = New-AzureStorageAccount $storageAccountName
}
...
if (!$service)
{
    $service = Execute-Command -Command { New-AzureService $serviceName }
}
...
$configurationPath = $configurationPath + ".final"
...
$diagnosticsConfigurationPath = $diagnosticsConfigurationPath + ".final"
